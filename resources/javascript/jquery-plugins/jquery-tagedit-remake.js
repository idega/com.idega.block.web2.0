...
(function($) {
	...
	$.fn.tagedit = function(options) {
		...
		*/
		...
		inputId = inputId == undefined ? "tagedit-input" : inputId;
		...
		*/
		function inputsToList() {
			...
			html += '<input type="text" name="'+baseName+'[]" value="" id="'+ inputId +'" disabled="disabled" class="tagedit-input-disabled" dir="'+options.direction+'"/>';
			html += '</li>';
			html += '</ul>';

			a = jQuery(inputIdSelector);
			elements
				.append(html)
				// Set function on the input
				.find(inputIdSelector)
					.each(function() {
						...
						$(this).bind('transformToTag', options.transform)
		...
		this.inputsToList = inputsToList;
		...
		*/
		...
		var data = {
				isNew : isNew,
				doEdit: this.doEdit,
				inputsToList : this.inputsToList,
				inputsToList : this.markAsDeleted,
				options : options
		}
		$(inputIdSelector).data("tag-options-data",data);
	}
})(jQuery);