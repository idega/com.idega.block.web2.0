...
function tree_component () {
	...
	if(typeof tree_component.inst == "undefined") {
		...
		tree_component.mouseup = function(event) {
			...
			if(_this.foreign === false && _this.drag && _this.drag.parentNode && _this.drag.parentNode == jQuery(_this.container).children("ul:eq(0)").get(0)) {
				...
				if(_this.moveType) {
					var tmp = tree_component.inst[jQuery(_this.moveRef).parents(".jsTree:eq(0)").attr("id")];
					if(tmp) { 
	...
	return {
		...
		init : function(elem, opts) {
			...
			this.container.addClass("jsTree");
		...
		moved : function (what, where, how, is_new, is_copy) {
			...
			if(!is_new) {
				var tmp = jQuery(what).parents(".jsTree:eq(0)");
				// if different trees
		...
		destroy : function() {
			...
			this.container.removeClass("jsTree").children("ul").removeClass("tree-" + this.settings.ui.theme_name).find("li").removeClass("leaf").removeClass("open").removeClass("closed").removeClass("last").children("a").removeClass("clicked");
