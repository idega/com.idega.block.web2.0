	...
	return {
		...
		init : function(elem, opts) {
			...
			if(this.settings.languages && this.settings.languages.length) {
				...
				var st = false;
				var id = this.container.attr("id") ? "#" + this.container.attr("id") : ".jsTree";
			...
			this.container.addClass("jsTree").css({ position: "relative" });
		...
		refresh : function (obj) {
			...
			if(this.settings.data.type == "xml_flat" || this.settings.data.type == "xml_nested") {
				...
				if(this.settings.data.json) {
					...
					var _this = this;
					jQuery.getJSON(this.settings.data.url, { id : 0 }, function (data) {
						var str = "";
						if(data.length) {
		...
		getJSON : function (nod) {
			if(!nod || jQuery(nod).size() == 0)	nod = this.container.find("li:eq(0)");
			else							nod = jQuery(nod);
				...
				if(this.settings.rules.draggable != "none" && this.settings.rules.dragrules != "none") {
					...
					jQuery(document)
						...
						.bind("mouseup", function (event) {
							...
							if(_this.drag && _this.drag.parentNode && _this.drag.parentNode == jQuery(_this.container).get(0)) {
						...
						.bind("mousemove", function (event) {
							...
							if(_this.isdown) {
								...
								if(!_this.appended) {
									...
									_this.po = jQuery(_this.drag).offsetParent().offset({scroll : false});
									_this.appended = true;
								}
								...
								var cnt = jQuery(event.target).parents(".jsTree:eq(0)");
								if(cnt.size() == 0) {
									if(_this.sto) clearTimeout(_this.sto);
									return false;
								}
								...
								if(event.target.tagName == "A" ) {
									var goTo = { 
										x : (jQuery(event.target).offset({scroll : false}).left - 1),
									...
									if(_this.checkMove(cnt.find("li.dragged"),jQuery(event.target),mov)) {
										if(mov == "inside")	jQuery("#marker").attr("src", _this.path + "images/plus.gif").width(11);
										else {
											if(cnt.hasClass("rtl"))	{ jQuery("#marker").attr("src", _this.path + "images/marker_rtl.gif").width(40); }
											else					{ jQuery("#marker").attr("src", _this.path + "images/marker.gif").width(40); }
										}
										_this.moveType	= mov;
		...
		checkMove : function (NODES, REF_NODE, TYPE) {
			...
			if(NODES.size() == 1) {
				...
				NODES.each(function (i) {
					...
					if(!_this.check("dragrules", [jQuery(this), mv, ref])) ok = false;
				});
		...
		open_branch : function (obj, disable_animation, callback) {
			...
			if(this.settings.data.async && obj.find("li").size() == 0) {
				...
				if(this.settings.data.type == "xml_flat" || this.settings.data.type == "xml_nested") {
					...
					jQuery.getJSON(this.settings.data.url, { id : obj.attr("id") }, function (data, textStatus) {
		...
		create : function (type) {
			...
			if(this.settings.rules.use_inline && this.settings.rules.metadata) {
				...
				if(typeof this.selected.metadata()["valid_children"] != "undefined") {
					if(jQuery.inArray(t, this.selected.metadata()["valid_children"]) == -1) return this.error("CREATE: NODE NOT A VALID CHILD");
				}
		...
		get_next : function() {
			...
			if(obj) {
				...
				else if(jQuery(obj).nextAll("li").size() > 0)	return this.hover_branch(obj.nextAll("li:eq(0)"));
