...
jQuery.fn.addSubGrid = function(t,row,pos) {
	return this.each(function(){
		...
		jQuery(td,t).html("<img src='"+ts.p.imgpath+"plus.gif'/>")
			.toggle( function(e) {
				...
				pID = jQuery("table:first",ts.grid.bDiv).attr("id");
				res = jQuery(this).parent();
				var atd= pos==1?'<td></td>':'';
				_id = jQuery(res).attr("id");
				...
				jQuery(this).parent().after( subdata+ "</div></td></tr>" );
				jQuery(".tablediv",ts).css("width", ts.grid.width-20+"px");
				...
				jQuery(this).parent().next().remove(".subgrid");
				jQuery(this).html("<img src='"+ts.p.imgpath+"plus.gif'/>");
			});
		row.appendChild(td);	
		//-------------------------
					...
					if(ts.p.retrieveMode === 'url') {
                        ...
                        if(ts.p.populateSubgridFromFunction != null) {
                            ...
                            ts.p.populateSubgridFromFunction(
                                {data: dp},
                                function(sxml) { subGridJXml(sxml, sid);}
                            );
                        }
                    }
                    /*</added>*/
		...
		var subGridJXml = function(sjxml, sbid){
			...
			if (sjxml){
				...
				var pID = jQuery("table:first",ts.grid.bDiv).attr("id")+"_";
