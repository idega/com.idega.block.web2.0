...
var MooRainbow = new Class({
	...
	rePosition: function() {
		...
		if (this.options.centerPosition) {
			var boxWidth = 390;
			var boxHeight = 310;
			
			var boxes = $$('div.' + this.options.prefix + 'box');
			if (boxes != null) {
				var boxSize = boxes[0].getSize();
				
				boxWidth = boxSize.size.x;
				boxHeight = boxSize.size.y;
			...
			left = (window.screen.width / 2) - (boxWidth / 2);
			height = (window.screen.height / 2) - (boxHeight / 2);
		}
		else {
			var coords = this.element.getCoordinates();
			left = coords.left;
			height = coords.top + coords.height + 1;
		}
